...
class TagBot:
    ...
    def reputation_is_enough(self, author_rep):
        ...
        rep = reputation(author_rep)
        return rep > self.config["MINIMUM_AUTHOR_REP"] and rep < self.config["MAXIMUM_AUTHOR_REP"]
...
def main():
    ...
    upvote_bot = TagBot(
        config,
        get_steem_conn(config["NODES"], config['POSTING_KEY'])
    )
    upvote_bot.run()
